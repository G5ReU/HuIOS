...
if (_data && _data.chars) {
    ...
    if (lastUserMsg && lastUserMsg.content && lastUserMsg.content.length > 4) {
        msgs.push({
            role: 'system',
            content: '【提示】如果你的回复是针对用户刚才某句具体的话，可以用<QUOTE>那句原话</QUOTE>引用它，让对话更有互动感。只引用用户说的原话，且必须与你的回复直接相关。'
        });
    }
    return msgs;
}
...
text = text.replace(/<SPL-IT>/gi, '<SPLIT>')
           ...
           .replace(/< SPLIT >/gi, '<SPLIT>')
           .replace(/<\/SPLIT>/gi, '<SPLIT>');  // </SPLIT> 也变成 <SPLIT>

// 第二步：把 <TAG>内容<TAG> 修复为 <TAG>内容</TAG>
...
_selfCloseTags.forEach(function(tag) {
    ...
    // 心声
var heartMatch = text.match(/<HEART>([\s\S]*?)(?:<\/HEART>|<HEART>)/i);
var stateMatch = text.match(/<STATE>([\s\S]*?)(?:<\/STATE>|<STATE>)/i);
var rateMatch = text.match(/<RATE>[^\d]*(\d+)[^\d]*<\/RATE>/);
